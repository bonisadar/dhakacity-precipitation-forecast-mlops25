...
def fetch_weather_data(
    ...
    print(
        f"Fetching weather data from {start_date} to {end_date} for lat: {latitude}, lon: {longitude}"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Weather data saved: {len(df)} rows | From {df['time'].min().date()} to {df['time'].max().date()}"
    )
